...
module PARSETREE : sig
  val signature :
    V4001_types.Parsetree.signature -> Parsetree.signature
  val structure :
    V4001_types.Parsetree.structure -> Parsetree.structure
  ...
  open Parsetree
  module T = V4001_types.Parsetree
  ...
  and package_type (l, list) =
    (l,
     List.map
       (fun (s,c) ->
         {s with txt =
             match s.txt with Longident.Lident s -> s | _ -> assert false},
         core_type c)
       list)
  ...
  and pattern_desc p =
    match p with
      ...
      | T.Ppat_construct (l, o, bool) ->
        Ppat_construct (l, option pattern o, bool)
      | T.Ppat_variant (l, o) ->
      ...
      | T.Ppat_type l -> Ppat_type l
      | T.Ppat_lazy p1 -> Ppat_lazy (pattern p1)
      | T.Ppat_unpack p -> Ppat_unpack p
...
*)
  ...
  and expression_desc e =
    match e with
      ...
      | T.Pexp_construct (l, o, bool) ->
        Pexp_construct (l, option expression o, bool)
      | T.Pexp_variant (l, o) ->
        Pexp_variant (l, option expression o)
      | T.Pexp_record (list, o) ->
        Pexp_record (List.map (fun (l, e) ->
          (l, expression e)) list, option expression o)
      | T.Pexp_field (e, l) ->
        Pexp_field (expression e, l)
      | T.Pexp_setfield (e1, l, e2) ->
        Pexp_setfield (expression e1, l, expression e2)
      ...
      | T.Pexp_for (s, e1, e2, d, e3) ->
        Pexp_for (s, expression e1, expression e2, d, expression e3)
      ...
      | T.Pexp_pack m -> Pexp_pack (module_expr m)
      | T.Pexp_open (l, e) -> Pexp_open (l, expression e)
  ...
  and type_declaration t =
    { ptype_params = List.map (fun s -> s) t.T.ptype_params;
  ...
  and type_kind t =
    match t with
      ...
      | T.Ptype_variant list ->
        Ptype_variant (List.map (fun (s, list, r, l) ->
          (s, List.map core_type list, Option.map core_type r, l)) list)
      | T.Ptype_record list ->
        Ptype_record (List.map (fun (s,m,c,l) -> (s, m, core_type c, l)) list)
  ...
  and class_type_desc c =
    match c with
        T.Pcty_constr (l, list) ->
          Pcty_constr (l, List.map core_type list)
      | T.Pcty_signature c -> Pcty_signature (class_signature c)
      | T.Pcty_fun (l, ct, cl) ->
        Pcty_fun (l, core_type ct, class_type cl)

  and class_signature cs =
    { pcsig_self = core_type cs.T.pcsig_self;
      pcsig_fields = List.map class_type_field cs.T.pcsig_fields;
      pcsig_loc = cs.T.pcsig_loc;
    }

  and class_type_field c = {
    pctf_desc = class_type_field_desc c.T.pctf_desc;
    pctf_loc = c.T.pctf_loc
  }

  and class_type_field_desc = function
    | T.Pctf_inher c -> Pctf_inher (class_type c)
    | T.Pctf_val (s, m, v, c) ->
        Pctf_val (s, mutable_flag m, virtual_flag v, core_type c)
    | T.Pctf_virt (s, p, c) ->
        Pctf_virt (s, private_flag p, core_type c)
    | T.Pctf_meth (s, p, c) ->
        Pctf_meth (s, private_flag p, core_type c)
    | T.Pctf_cstr (c1, c2) ->
        Pctf_cstr (core_type c1, core_type c2)
...
and class_expr_desc c =
  match c with
    T.Pcl_constr (l, list) ->
      Pcl_constr (l, List.map core_type list)
  | T.Pcl_structure c ->
...
and class_structure cs = {
  pcstr_pat = pattern cs.T.pcstr_pat;
  pcstr_fields =  List.map class_field cs.T.pcstr_fields;
}

and class_field c = {
    pcf_desc = class_field_desc c.T.pcf_desc;
    pcf_loc = c.T.pcf_loc
}
...
and class_field_desc = function
  ...
  | T.Pcf_valvirt (s, mf, c) ->
      Pcf_valvirt (s, mutable_flag mf, core_type c)
  | T.Pcf_val (s, mf, ovf, e) ->
      Pcf_val (s, mutable_flag mf, override_flag ovf, expression e)
  | T.Pcf_virt  (s, pf, c) ->
      Pcf_virt (s, private_flag pf, core_type c)
  | T.Pcf_meth (s, pf, ovf, e) ->
      Pcf_meth (s , private_flag pf, override_flag ovf, expression e)
  | T.Pcf_constr (c1, c2) ->
      Pcf_constr (core_type c1, core_type c2)
(*
  | T.Pcf_let  (r, list) ->
      Pcf_let ( rec_flag r, List.map (fun (pat, e)  ->
            (pattern pat, expression e)) list)
*)
...
and signature_item_desc si =
  ...
  | T.Psig_type list ->
      Psig_type (List.map (fun (s, t) -> (s, type_declaration t)) list)
...
and with_constraint w =
  ...
  | T.Pwith_module l -> Pwith_module (l)
  | T.Pwith_typesubst t -> Pwith_typesubst (type_declaration t)
  | T.Pwith_modsubst l ->  Pwith_modsubst (l)
...
and module_expr_desc me =
  ...
  | T.Pmod_unpack e -> Pmod_unpack (expression e)
...
and structure_item_desc si =
  ...
  | T.Pstr_exn_rebind (s, l) -> Pstr_exn_rebind (s, l)
...
(*
  val reset : unit -> unit
*)
  val t : V4001_types.Ident.t -> Ident.t

end = struct

  module T = V4001_types.Ident
  open Ident
...
module PATH : sig
  ...
  val t : V4001_types.Path.t -> Path.t

end = struct

  module T = V4001_types.Path
  open Path
...
module CMI = struct
  ...
  open V4001_input_ast.AST

module Primitive : sig

  val description : V4001_types.Primitive.description -> Primitive.description

end = struct

    module T = V4001_types.Primitive
    open Primitive
...
module TYPES : sig
    ...
    val signature :
      V4001_types.Types.signature_item list -> Types.signature_item list

    val record_representation :
      V4001_types.Types.record_representation ->
    ...
    and type_desc desc =
      ...
      | T.Tunivar v -> Tunivar v
      | T.Tpoly (t, list) -> Tpoly (type_expr t, List.map type_expr list)
      | T.Tpackage (p, sl, tl) ->
          Tpackage (
            PATH.t p,
            List.map (function Longident.Lident s -> s | _ -> assert false) sl,
            List.map type_expr tl
          )
    ...
    and type_kind t =
      ...
      | T.Type_variant list -> Type_variant (List.map (fun (s, tlist, t) ->
                (IDENT.t s, List.map type_expr tlist, Option.map type_expr t)) list)
      | T.Type_record (list, rr) ->
          Type_record (List.map (fun (s, mf, t) ->
                (IDENT.t s, mf, type_expr t)) list,
    ...
    and exception_declaration ed = List.map type_expr ed.T.exn_args

    and module_type decl =
      match decl with
        T.Mty_ident p -> Mty_ident (PATH.t p)
      | T.Mty_signature s ->
          Mty_signature (signature s)
      | T.Mty_functor (id, m1, m2) ->
          Mty_functor (IDENT.t id, module_type m1, module_type m2)

    and modtype_declaration decl =
      match decl with
        T.Modtype_abstract -> Modtype_abstract
      | T.Modtype_manifest mt ->
...
module Cmi_format : sig
    ...
    val pers_flags :
      V4001_types.Cmi_format.pers_flags -> Cmi_format.pers_flags
    ;;

  end = struct

    module T = V4001_types.Cmi_format
    open Cmi_format
...
let input_cmi_file ic magic =
  if magic <> V4001_types.cmi_magic_number then
    raise (Error (No_Such_Magic ("cmi", magic)));

  CMI.Ident.reset ();
  CMI.Types.reset ();

  let (cmi_name, cmi_sign) = (input_value ic : string *  V4001_types.Types.signature_item list) in
  let cmi_crcs = (input_value ic : (string * Digest.t) list) in
  let cmi_flags = (input_value ic : V4001_types.Cmi_format.pers_flags list) in
...
let input_cmi ic =
  let (cmi_name, cmi_sign) =
    (input_value ic : string *  V4001_types.Types.signature_item list) in
  ctx := {
...
let input_ast_intf ic =
  ...
  let intf = (input_value ic : V4001_types.Parsetree.signature) in
  let intf = PARSETREE.signature intf in
  (input_name, intf)

let input_ast_impl ic =
  let input_name = (input_value ic : string) in
  let impl = (input_value ic : V4001_types.Parsetree.structure) in
